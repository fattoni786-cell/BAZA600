...
def game_vibe_keyboard(
    ...
    all_vibes = [
        (tag, label)
        for tag, label in get_public_vibes("game").items()
        if any(
            tag in game.get("tags", []) and game["title"] not in excluded_titles
            for game in games
        )
    ]
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    text=label,
                    callback_data=f"fast_game_vibe:{tag}",
                )
            ]
        )

    keyboard.append(
        [InlineKeyboardButton(text="🔄 Обновить вайбы", callback_data="refresh_game_vibes")]
    )
    return InlineKeyboardMarkup(inline_keyboard=keyboard)


def another_game_keyboard(
    vibe: str | None,
    title: str | None = None,
    ...
    if collection_name and show_next_collection:
        keyboard.append(
            [
                InlineKeyboardButton(
                    ...
                    callback_data="next_game_collection",
                )
            ]
        )
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    text="🎲 Другая игра",
                    callback_data=f"another_game:{vibe}",
                )
            ]
        )

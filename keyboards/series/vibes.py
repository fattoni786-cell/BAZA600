...
def series_vibe_keyboard(excluded_titles: set[str] | None = None):
    keyboard = []
    excluded_titles = excluded_titles or set()

    series_items = load_series(include_premium_collection_only=False)
    all_vibes = [
        (tag, label)
        for tag, label in get_public_vibes("series").items()
        if any(
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    text=label,
                    callback_data=f"fast_series_vibe:{tag}",
                )
            ]
        )

    keyboard.append(
        [InlineKeyboardButton(text="🔄 Обновить вайбы", callback_data="refresh_series_vibes")]
    )
    return InlineKeyboardMarkup(inline_keyboard=keyboard)


def another_series_keyboard(
    vibe: str | None,
    title: str | None = None,
    ...
    if collection_name and show_next_collection:
        keyboard.append(
            [
                InlineKeyboardButton(
                    ...
                    callback_data="next_series_collection",
                )
            ]
        )
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    text="🎲 Другой сериал",
                    callback_data=f"another_series:{vibe}",
                )
            ]
        )

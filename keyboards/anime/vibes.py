...
def anime_vibe_keyboard(excluded_titles: set[str] | None = None):
    keyboard = []
    excluded_titles = excluded_titles or set()

    anime_items = load_anime(include_premium_collection_only=False)
    all_vibes = [
        (tag, label)
        for tag, label in get_public_vibes("anime").items()
        if any(
            tag in item.get("tags", []) and item["title"] not in excluded_titles
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    text=label,
                    callback_data=f"fast_anime_vibe:{tag}",
                )
            ]
        )

    keyboard.append(
        [InlineKeyboardButton(text="🔄 Обновить вайбы", callback_data="refresh_anime_vibes")]
    )
    return InlineKeyboardMarkup(inline_keyboard=keyboard)


def another_anime_keyboard(
    vibe: str | None,
    title: str | None = None,
    ...
    if collection_name and show_next_collection:
        keyboard.append(
            [
                InlineKeyboardButton(
                    ...
                    callback_data="next_anime_collection",
                )
            ]
        )
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    text="🎲 Давай другое аниме",
                    callback_data=f"another_anime:{vibe}",
                )
            ]
        )
